<template>
  <div class="border-2 hover:border-primary cursor-pointer">
    <div class="p-5 grid grid-cols-5 grid-gap-2 text-sm">
      ...
      <div>
        <div>
          <i :style="`background-color: ${shantytown.fieldType.color}`"></i>
      ...
      <div>
        <span v-if="shantytown.populationTotal === null" class="secondary"
        ...
        <span v-else>
          ...
          <span class="secondary"
            >au
            {{
              formatDate(shantytown.updatedAt || shantytown.builtAt, "d/m/y")
            }}</span
          >
        </span>
      </div>
      ...
      <div>
        ...
        <div v-else>
          <li v-for="status in shantytown.justiceStatuses" :key="status.label">
            {{ status.label }}
            <span v-if="status.date" class="secondary"
              ><br />le {{ formatDate(status.date, "d/m/y") }}</span
            >
          </li>
        </div>
      </div>
      ...
      <div>
        ...
        <div>Evacuation des déchets: {{ shantytown.trashEvacuation }}</div>
      </div>
    </div>
